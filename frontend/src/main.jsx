...
const uiTranslations = {
  fr: {
    ...
    caseTitle: "Saisir le cas patient",
    answerPlaceholder: "Votre reponse",
    searchPlaceholder: "Rechercher...",
    stepCase: "Cas initial",
  ...
  en: {
    ...
    caseTitle: "Enter patient case",
    answerPlaceholder: "Your answer",
    searchPlaceholder: "Search...",
    stepCase: "Initial case",
    stepQuestions: "Questions",
    stepReview: "Physician",
    stepReport: "Report",
    historyDone: "Done",
  },
  es: {
    ...
    caseTitle: "Ingresar el caso del paciente",
    answerPlaceholder: "Su respuesta",
    searchPlaceholder: "Buscar...",
    stepCase: "Caso inicial",
    stepQuestions: "Preguntas",
    stepReview: "Medico",
    stepReport: "Informe",
    historyDone: "Terminado",
  },
  ar: {
    newStatus: "New",
    caseStep: "Step 1",
...
const steps = [
  { key: "case", labelKey: "stepCase", icon: ClipboardPlus },
  { key: "questions", labelKey: "stepQuestions", icon: UserRound },
  { key: "review", labelKey: "stepReview", icon: Stethoscope },
  { key: "report", labelKey: "stepReport", icon: FileText },
];
...
function Stepper({ activeStep, ui }) {
...
function getDisplayedQuestionCount(data) {
  if (!data) return 0;

  const values = [
    data.interrupt?.question_number,
    data.state?.question_count,
    data.state?.questions?.length,
    data.state?.patient_answers?.length,
  ].filter((value) => Number.isFinite(Number(value)));

  return Math.min(5, Math.max(0, ...values.map(Number)));
}

function StatusCard({ data, t, ui, lang, onLangChange, onOpenDashboard }) {
  const questionCount = getDisplayedQuestionCount(data);
  const status =
  ...
  return (
    <aside className="statusPanel">
      <div className="sideControls">
        ...
        <button className="utilityButton" onClick={onOpenDashboard}>
          <BarChart3 size={16} />
          {t.dashboard}
        </button>
      </div>
  ...
  return (
    <section className="workPanel enter">
      <div className="panelHeader">
        <div>
          <p className="eyebrow">{ui.caseStep}</p>
          <h2>{ui.caseTitle}</h2>
        </div>
        <Activity size={24} />
      </div>
...
function QuestionForm({ data, onResume, loading, t, ui }) {
  const [answer, setAnswer] = useState("");
  const interrupt = data.interrupt;
  const answers = data.state?.patient_answers || [];
  const urgency = data.urgency;
  const questionNumber = getDisplayedQuestionCount(data) || 1;
  const questionText = (interrupt.question || "").replace(
    /^Question\s+\d+\s*\/\s*5\s*:\s*/i,
    ""
  );
    ...
    <section className="workPanel enter">
      ...
      <div className="clinicalBox">
        ...
        <div className="clinicalContent">
          {parseReport(interrupt.diagnostic_summary || "")}
        </div>
      </div>
      ...
      <div className="clinicalBox accent">
        <h3>{t.interimCare}</h3>
        <div className="clinicalContent">
          {parseReport(interrupt.interim_care || "")}
        </div>
      </div>
...
function parseReport(report) {
  ...
  const flushList = () => {
    ...
    elements.push(
      <ul className="reportList" key={`list-${elements.length}`}>
  ...
  lines.forEach((line, lineIndex) => {
    ...
    const underlinedHeading = /^[=\-]{3,}$/.test(line);
    const bullet = line.match(/^[-*]\s+(.+)$/);
    const boldHeading = line.match(/^\*\*(.+?)\*\*:?\s*$/);
    const sectionLine = line.match(/^([^:]{2,40}):\s*(.*)$/);
    const isSectionLine =
      sectionLine &&
      sectionLine[1] === sectionLine[1].toLocaleUpperCase("fr-FR");

    if (underlinedHeading) return;

    if (lineIndex === 0 && /^rapport final/i.test(line)) {
      flushList();
      elements.push(
        <h2 key={`title-${elements.length}`}>{cleanReportText(line)}</h2>
      );
      return;
    }
    ...
    if (heading || boldHeading) {
      ...
      elements.push(
        <h3 key={`heading-${elements.length}`}>
          {cleanReportText((heading || boldHeading)[1])}
        </h3>
      );
      return;
    }
    ...
    if (bullet) {
      listItems.push(cleanReportText(bullet[1]));
      return;
    }
    ...
    if (isSectionLine) {
      flushList();
      elements.push(
        <h3 key={`section-${elements.length}`}>
          {cleanReportText(sectionLine[1])}
        </h3>
      );
      if (sectionLine[2]) {
        elements.push(
          <p key={`section-text-${elements.length}`}>
            {cleanReportText(sectionLine[2])}
          </p>
        );
      }
      return;
    ...
    flushList();
    elements.push(
      <p key={`paragraph-${elements.length}`}>{cleanReportText(line)}</p>
    );
  });
...
function ReportView({ data, onReset, t, ui }) {
  const report = data.state?.final_report || "";
  const [showCompare, setShowCompare] = useState(false);

  const formattedReport = useMemo(() => parseReport(report), [report]);
  ...
  const generatePDF = () => {
    import("jspdf").then(({ jsPDF }) => {
      ...
      lines.forEach((line) => {
        ...
        const cleanLine = cleanReportText(line.replace(/[#=]/g, ""));
        if (cleanLine) {
  ...
  return (
    <section className="workPanel reportPanel enter">
      <div className="panelHeader">
        <div>
          <p className="eyebrow">{ui.finalStep}</p>
          <h2>{t.finalReport}</h2>
        </div>
        <FileText size={24} />
      </div>
...
const HistoryPanel = forwardRef(function HistoryPanel(
  { onLoadSession, currentThreadId, t, ui },
  ref
) {
  ...
  return (
    <main className="appShell">
      ...
      <div className="mainColumn">
        ...
        <Stepper activeStep={activeStep} ui={ui} />
        ...
        {data?.state?.final_report && (
          <ReportView data={data} onReset={reset} t={t} ui={ui} />
        )}
      </div>
    </main>
  );
